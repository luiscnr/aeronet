...
def do_comparison_bal_multi_olci():
    ...
    dir_out = '/store/COP2-OC-TAC/BAL_Evolutions/COMPARISON_MULTI_OLCI/RRS560'
    ...
    while date_here<=end_date:
        ...
        if os.path.exists(dir_olci) and os.path.exists(dir_multi):
            ...
            file_olci =os.path.join(dir_olci,f'O{year}{jday}-rrs560-bal-fr.nc')
            # chla,rrs443,rrs490,rrs510,rrs560,rrs665
            file_multi = os.path.join(dir_multi,f'C{year}{jday}-rrs560-bal-hr.nc')

            if os.path.exists(file_multi) and os.path.exists(file_olci):
                print(f'[INFO] Making date: {date_here}')
                file_out = os.path.join(dir_out,f'Comparison_RRS560_{year}{jday}.csv')
                make_comparison_impl(file_grid,file_multi,file_olci,file_out,'RRS560','RRS560')
